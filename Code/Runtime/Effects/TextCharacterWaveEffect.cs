...
{
    [CreateAssetMenu(menuName="Text Effects/Wave/Character Wave Effect", fileName="New Character Wave Effect")]
    public class TextCharacterWaveEffect : TextEffect
    {
        ...
        public override void ApplyEffect(TMP_Text textComponent, TextEffectsParser parser)
        {
            ...
            for (int i = 0; i < totalCharacterCount; ++i)
            {
                ...
                for (int j = 0; j < 4; ++j)
                {
                    ...
                    meshInfo.vertices[k] = origin + new Vector3(0f, Mathf.Sin(Time.time * speedMultiplier + i * curveMultiplier) * amplitude, 0f);
                }
            }
        }
    }
